...
class cloudstudio_view extends \table_sql {
    ...
    public function __construct($uniqueid, $cmid, $userid, $cloudstudio) {
        ...
        if ($download) {
            ...
            if ($this->userid) {
                ...
                $filename = get_string('report_filename', 'mod_cloudstudio', fullname($user));
            } else {
                $geral = get_string('report_filename_geral', 'mod_cloudstudio');
                $filename = get_string('report_filename', 'mod_cloudstudio', $geral);
            }
            $this->is_downloading($download, $filename, $cloudstudio->name);
        }
        ...
        if ($this->userid) {
            ...
            $headers = [
                get_string('report_userid', 'mod_cloudstudio'),
                get_string('report_nome', 'mod_cloudstudio'),
                get_string('report_email', 'mod_cloudstudio'),
                get_string('report_tempo', 'mod_cloudstudio'),
                get_string('report_duracao', 'mod_cloudstudio'),
                get_string('report_porcentagem', 'mod_cloudstudio'),
                get_string('report_mapa', 'mod_cloudstudio'),
                get_string('report_comecou', 'mod_cloudstudio'),
    ...
    public function col_extra($linha) {
        $profileurl = new \moodle_url('/mod/cloudstudio/report.php?', ['id' => $linha->cm_id, 'u' => $linha->user_id]);
        return \html_writer::link($profileurl, get_string('report_all', 'mod_cloudstudio'));
    }
